...
def vision_control_update(
    ...
    selection_strategy="first",  # "first", "largest", or "most_confident"
):
    ret, frame = cap.read()
    if not ret:
    ...
    if not results or not hasattr(results[0], 'boxes') or not results[0].boxes:
        ...
        if candidates:
            ...
            elif selection_strategy == "most_confident":
                selected = max(candidates, key=lambda c: c["conf"])
            else:  # "first" or anything else
...
def p_control_loop(robot, keyboard, target_positions, start_positions, current_x, current_y, kp=0.5, control_freq=50, model=None, cap=None, vision_mode=False, target_objects=["mouse"], selection_strategy="first"):
    """
    P control loop
    ...
    csv_log_interval = 2.0  # Log every 2 seconds
    ...
    # Write CSV header
    csv_header = ['timestamp', 'shoulder_pan', 'shoulder_lift', 'elbow_flex', 'wrist_flex', 'wrist_roll', 'gripper']
    csv_writer.writerow(csv_header)
    csv_file.flush()
    
    print(f"Starting P control loop, control frequency: {control_freq}Hz, proportional gain: {kp}")
    print(f"Motor positions will be logged to: {csv_filename}")
        ...
        try:
            ...
            if current_time - last_csv_log_time >= csv_log_interval:
                csv_row = [
                    ...
                    f"{current_positions.get('shoulder_pan', 0):.3f}",
                    f"{current_positions.get('shoulder_lift', 0):.3f}",
                    f"{current_positions.get('elbow_flex', 0):.3f}",
                    f"{current_positions.get('wrist_flex', 0):.3f}",
                    f"{current_positions.get('wrist_roll', 0):.3f}",
                    f"{current_positions.get('gripper', 0):.3f}"
                ]
...
def main():
    ...
    try:
        ...
        # Configure robot
        robot_config = SO101FollowerConfig(port=port,id = 'follower_qqq')
        ...
        model = model.to(device) # move the yolo to gpu
        ...
        # Choose object selection strategy for multiple detections
        print("\nSelection strategy options:")
        print("- first: follow the first detected target (current behavior)")
        print("- largest: follow the target with the largest bounding box")
        print("- most_confident: follow the target with highest confidence")
        strategy_input = input("Enter selection strategy (first/largest/most_confident, default=first): ").strip().lower()
        if strategy_input not in {"first", "largest", "most_confident"}:
            strategy_input = "first"
        ...
        p_control_loop(
            ...
            selection_strategy=strategy_input,
        )
